...
class ComparisonReport:
    ...
    def summary_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "feature_coverage": {
                ...
                "main_percentage": round(self.main.tracks_with_features / self.main.track_count * 100, 1),
                "deleted_percentage": round(self.deleted.tracks_with_features / self.deleted.track_count * 100, 1),
            },
            "audio_features": {
                "main_median_bpm": median(self.main.bpm_values) if self.main.bpm_values else None,
                "deleted_median_bpm": median(self.deleted.bpm_values) if self.deleted.bpm_values else None,
                "main_median_lufs": median(self.main.lufs_values) if self.main.lufs_values else None,
                "deleted_median_lufs": median(self.deleted.lufs_values) if self.deleted.lufs_values else None,
                "main_median_energy": median(self.main.energy_values) if self.main.energy_values else None,
                "deleted_median_energy": median(self.deleted.energy_values) if self.deleted.energy_values else None,
            },
            "metadata_signals": {
                "main_median_year": median(self.main.album_years) if self.main.album_years else None,
                "deleted_median_year": median(self.deleted.album_years) if self.deleted.album_years else None,
                "main_compilation_rate": round(self.main.compilation_like_count / self.main.track_count * 100, 1),
                "deleted_compilation_rate": round(self.deleted.compilation_like_count / self.deleted.track_count * 100, 1),
            },
            "user_feedback": {
    ...
    def filtering_recommendations(self) -> dict[str, Any]:
        ...
        if stats["user_feedback"]["deleted_disliked_count"] > stats["user_feedback"]["main_disliked_count"]:
            recommendations["must_block"].append("YM user dislikes")
        
        # Check if missing features is a strong signal
        deleted_missing_rate = (len(self.deleted.tracks_without_features) / self.deleted.track_count) * 100
        ...
        if deleted_missing_rate > main_missing_rate + 10:
            ...
            recommendations["soft_score_signals"].append("Missing audio features (moderate penalty)")
        else:
            recommendations["signals_too_weak"].append("Missing audio features (insufficient signal)")
            
        # Metadata year signal
        if (stats["metadata_signals"]["main_median_year"] and 
            stats["metadata_signals"]["deleted_median_year"]):
            year_diff = stats["metadata_signals"]["main_median_year"] - stats["metadata_signals"]["deleted_median_year"]
            if year_diff >= 2:
                recommendations["soft_score_signals"].append(f"Album year (prefer newer, ~{year_diff} year bias)")
            else:
                recommendations["signals_too_weak"].append("Album year (weak signal)")
        
        # Compilation signal
        comp_diff = stats["metadata_signals"]["main_compilation_rate"] - stats["metadata_signals"]["deleted_compilation_rate"]
        if comp_diff < -10:
...
async def analyze_playlist(
    ...
    print(f"  {playlist.name}: {len(playlist.local_track_ids)}/{len(playlist.ym_track_ids)} tracks in local DB")
    ...
    for track_id in playlist.local_track_ids:
        async with session_factory() as session:
            ...
            if meta := meta_row.scalars().first():
                ...
                if (meta.album_title and 
                    any(word in meta.album_title.lower() for word in 
                        ["compilation", "best", "hits", "collection", "various", "mixed"])):
    ...
    print(f"  {playlist.name}: {playlist.tracks_with_files} have files, {playlist.tracks_with_features} have features")
    print(f"  {playlist.name}: {len(playlist.tracks_without_features)} missing features")
    print(f"  {playlist.name}: {len(playlist.liked_intersect)} liked, {len(playlist.disliked_intersect)} disliked")
...
async def main() -> None:
    ...
    parser.add_argument("--user-id", default=settings.yandex_music_user_id or "250905515", help="YM user ID")
    parser.add_argument("--output-report", action="store_true", help="Write analysis report to file")
    ...
    try:
        ...
        deleted_name, deleted_tracks = await fetch_playlist_tracks(api, args.user_id, args.deleted_kind)
        ...
        print(f"\nFEATURE COVERAGE:")
        print(f"  Main: {stats['feature_coverage']['main_with_features']} ({stats['feature_coverage']['main_percentage']}%)")
        print(f"  Deleted: {stats['feature_coverage']['deleted_with_features']} ({stats['feature_coverage']['deleted_percentage']}%)")
        
        if stats['audio_features']['main_median_bpm'] and stats['audio_features']['deleted_median_bpm']:
            print(f"\nAUDIO FEATURES:")
        ...
        print(f"  Main compilation rate: {stats['metadata_signals']['main_compilation_rate']:.1f}%")
        print(f"  Deleted compilation rate: {stats['metadata_signals']['deleted_compilation_rate']:.1f}%")
