-- ============================================================================
-- DJ Techno Set Builder — Database Schema v6
-- PostgreSQL 16+ / pgvector 0.7+
--
-- Полный цикл: ingest → DSP/ML анализ → transitions → сборка сетов → экспорт
--
-- Changelog v5 → v6 (по итогам второго code review):
--   • spotify_audio_features — удалён денормализованный spotify_track_id
--     (integrity gap: track_id=2 мог ссылаться на spotify_track_id трека 1)
--   • track_sections — CHECK на bounded range_ms + CHECK section_duration_ms = width(range_ms)
--   • dj_beatgrid — partial unique index на is_canonical=true (один canonical на трек)
--   • HNSW chroma комментарий: "похожих по спектру" → "похожих по гармоническому профилю"
--
-- Changelog v4 → v5 (self-review):
--   • FIX CRITICAL: providers перемещён ВЫШЕ track_genres (forward reference fix)
--   • camelot_distance() и key_distance_weighted() — IMMUTABLE → STABLE (читают key_edges)
--   • audio_assets.source_run_id — добавлен FK → feature_extraction_runs
--   • raw_provider_responses.provider_id — добавлен FK → providers
--   • track_audio_features_computed.key_code — добавлен FK → keys
--   • beatport_metadata.key_code — добавлен FK → keys
--   • dj_set_items.in_section_id/out_section_id — composite FK (section_id, track_id)
--   • spotify_metadata.spotify_album_id — добавлен FK → spotify_album_metadata
--   • dj_set_items — добавлены индексы (set_version_id, track_id)
--   • dj_set_versions — добавлен индекс на set_id
--   • dj_set_constraints — добавлен индекс на set_version_id
--   • track_artists — добавлен обратный индекс на artist_id
--   • track_releases — добавлен обратный индекс на release_id
--
-- Changelog v3 → v4 (по итогам code review):
--   • track_genres.source_provider_id — добавлен FK на providers
--   • provider_track_ids — NULLS NOT DISTINCT на UNIQUE constraint
--   • spotify_audio_features.spotify_track_id — FK на spotify_metadata (удалён в v6)
--   • raw_provider_responses — добавлен FK track_id → tracks (PG 12+ поддерживает)
--   • track_sections — составной UNIQUE (section_id, track_id) для composite FK
--   • transitions.from_section_id / to_section_id — composite FK (section_id, track_id)
--   • embedding dim validation — CHECK + trigger на vector_dims = embedding_types.dim
--   • dj_beatgrid — добавлен source_app для мультиисточности (Traktor/Rekordbox/djay)
--   • fingerprint_sha1 — CHECK на длину (20 bytes)
--   • checksum_sha256 — CHECK на длину (32 bytes)
--   • track_sections.section_duration_ms — CHECK на соответствие range_ms
--   • Триггеры updated_at — DROP IF EXISTS для идемпотентности
--   • Changelog v3 исправлен: embedding_types — registry + единая таблица (не per-type)
--
-- Changelog v2 → v3:
--   • track_genres PK — surrogate PK + UNIQUE NULLS NOT DISTINCT
--   • track_embeddings — registry embedding_types + единая таблица с partial HNSW
...
-- ════════════════════════════════════════════════════════════════════════════
-- 2. Providers (перемещены СЮДА из секции 3, до track_genres)
-- ════════════════════════════════════════════════════════════════════════════

-- FIX v5: providers определяется ДО track_genres, чтобы FK
-- track_genres.source_provider_id → providers не ломал DDL.
...
CREATE TABLE tracks (
    ...
    fingerprint_sha1  bytea         NOT NULL CHECK (octet_length(fingerprint_sha1) = 20),
    title             text          NOT NULL,
    title_sort        text,
    duration_ms       integer       NOT NULL CHECK (duration_ms > 0),
    status            smallint      NOT NULL DEFAULT 0 CHECK (status IN (0, 1)),
        -- 0=active, 1=broken/incomplete (файл повреждён, метаданные неполные)
...
-- FIX v5: обратный индекс — поиск треков по артисту
...
-- FIX v5: обратный индекс — поиск треков по релизу
CREATE INDEX idx_track_releases_release ON track_releases (release_id);
...
-- FIX v5: FK на providers теперь валиден — providers определены выше
CREATE TABLE track_genres (
    ...
    source_provider_id smallint   REFERENCES providers ON DELETE SET NULL,
    created_at         timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE provider_track_ids (
    ...
    CONSTRAINT provider_track_ids_uq
        UNIQUE NULLS NOT DISTINCT (provider_id, provider_track_id, provider_country)
);
...
-- FIX v5: добавлен FK provider_id → providers
...
COMMENT ON TABLE raw_provider_responses IS
    ...
    'FK track_id → tracks поддерживается в PG 12+ для partitioned таблиц.';


-- ════════════════════════════════════════════════════════════════════════════
-- 5. Provider metadata (slim — часто используемые поля + jsonb extra)
-- ════════════════════════════════════════════════════════════════════════════

-- ▸ Spotify album metadata (определяется ДО spotify_metadata для FK)
...
-- FIX v5: spotify_album_id — добавлен FK → spotify_album_metadata
...
CREATE TABLE spotify_metadata (
    ...
    spotify_album_id   text       REFERENCES spotify_album_metadata ON DELETE SET NULL,
...
-- FIX v6: удалён spotify_track_id — он денормализован и создаёт integrity gap.
-- track_id=2 мог ссылаться на spotify_track_id, принадлежащий track_id=1 в spotify_metadata.
-- Для получения spotify_track_id: JOIN spotify_metadata USING (track_id).
...
-- ▸ Beatport
-- FIX v5: beatport_metadata.key_code — добавлен FK → keys
-- Примечание: keys определена ниже, FK добавляется через ALTER TABLE после создания keys.
...
-- ════════════════════════════════════════════════════════════════════════════
-- 6. Audio assets (originals + Demucs stems + previews)
-- ════════════════════════════════════════════════════════════════════════════

-- Примечание: FK source_run_id → feature_extraction_runs добавляется через ALTER TABLE
-- после создания feature_extraction_runs (определена ниже).
...
-- FIX v5: отложенный FK — audio_assets.source_run_id → feature_extraction_runs
ALTER TABLE audio_assets
    ADD CONSTRAINT fk_audio_assets_source_run
    FOREIGN KEY (source_run_id) REFERENCES feature_extraction_runs ON DELETE SET NULL;
...
-- ════════════════════════════════════════════════════════════════════════════
-- 8. Harmony model (keys + compatibility graph)
-- ════════════════════════════════════════════════════════════════════════════
...
-- FIX v5: отложенные FK для key_code → keys
ALTER TABLE beatport_metadata
    ADD CONSTRAINT fk_beatport_key FOREIGN KEY (key_code) REFERENCES keys ON DELETE SET NULL;
...
-- ════════════════════════════════════════════════════════════════════════════
-- 9. Computed audio features (DSP/ML results)
-- ════════════════════════════════════════════════════════════════════════════

-- FIX v5: key_code — добавлен FK → keys
CREATE TABLE track_audio_features_computed (
    ...
    -- ── Tonal / Harmonic ──
    key_code              smallint   NOT NULL REFERENCES keys CHECK (key_code BETWEEN 0 AND 23),
...
-- FIX v6: HNSW по chroma для поиска "похожих по гармоническому профилю"
-- (chroma(12) — это harmonic pitch class profile, не timbral/spectral descriptor)
...
-- ════════════════════════════════════════════════════════════════════════════
-- 10. Track sections (structural segmentation)
-- ════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE track_sections (
    ...
    -- FIX v6: range_ms ОБЯЗАН быть bounded (не (-inf, x) или (x, +inf))
    CONSTRAINT sections_range_bounded
        CHECK (NOT upper_inf(range_ms) AND NOT lower_inf(range_ms)),
    -- FIX v6: section_duration_ms ОБЯЗАН совпадать с шириной range_ms
    CONSTRAINT sections_duration_matches_range
        CHECK (section_duration_ms = upper(range_ms) - lower(range_ms)),
    ...
    -- FIX v4: composite unique для composite FK из transitions
    -- Гарантирует, что FK (section_id, track_id) однозначно ссылается на секцию нужного трека
    CONSTRAINT sections_track_uq UNIQUE (section_id, track_id)
);
...
-- ════════════════════════════════════════════════════════════════════════════
-- 11. Timeseries references (frame-level → object storage)
-- ════════════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════════════
-- 12. Transition candidates (лёгкий pre-filter)
-- ════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE transitions (
    ...
    -- FIX v4: composite FK — section_id ОБЯЗАН принадлежать соответствующему track_id.
    -- Использует UNIQUE (section_id, track_id) из track_sections.
    CONSTRAINT fk_from_section FOREIGN KEY (from_section_id, from_track_id)
        REFERENCES track_sections (section_id, track_id) ON DELETE SET NULL,
    CONSTRAINT fk_to_section FOREIGN KEY (to_section_id, to_track_id)
        REFERENCES track_sections (section_id, track_id) ON DELETE SET NULL,
...
-- ════════════════════════════════════════════════════════════════════════════
-- 14. Embeddings (registry + единая таблица + dim validation trigger)
-- ════════════════════════════════════════════════════════════════════════════

-- Registry embedding_types + единая таблица track_embeddings.
-- Размерность vector валидируется триггером (не per-type таблицы).
-- HNSW-индексы создаются per type через partial indexes.
...
COMMENT ON TABLE embedding_types IS
    ...
    'dim используется для валидации vector_dims в trigger trg_check_embedding_dim.';
...
COMMENT ON TABLE track_embeddings IS
    ...
    'Единая таблица с registry embedding_types (не per-type таблицы). '
    'Размерность vector валидируется триггером trg_check_embedding_dim. '
...
-- FIX v4: триггер для валидации vector_dims = embedding_types.dim
CREATE OR REPLACE FUNCTION trg_check_embedding_dim()
RETURNS trigger
LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
    ...
    WHERE embedding_type = NEW.embedding_type;

    IF expected_dim IS NULL THEN
        RAISE EXCEPTION 'embedding_type "%" not found in embedding_types', NEW.embedding_type;
    END IF;

    IF vector_dims(NEW.vector) <> expected_dim THEN
        RAISE EXCEPTION 'vector dimension mismatch: got %, expected % for type "%"',
            vector_dims(NEW.vector), expected_dim, NEW.embedding_type;
    END IF;
...
CREATE TRIGGER trg_embeddings_dim_check
    BEFORE INSERT OR UPDATE ON track_embeddings
    FOR EACH ROW EXECUTE FUNCTION trg_check_embedding_dim();


-- ════════════════════════════════════════════════════════════════════════════
-- 15. DJ layer (library + beatgrid + cues + loops + playlists)
-- ════════════════════════════════════════════════════════════════════════════
...
-- FIX v4: добавлен source_app — Traktor, rekordbox и generated grids могут различаться.
-- PK теперь составной (track_id, source_app) вместо просто track_id.
CREATE TABLE dj_beatgrid (
    beatgrid_id        bigint     GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    track_id           integer    NOT NULL REFERENCES tracks ON DELETE CASCADE,
    source_app         smallint   NOT NULL CHECK (source_app BETWEEN 1 AND 5),
        -- 1=traktor, 2=rekordbox, 3=djay, 4=import, 5=generated
    bpm                real       NOT NULL CHECK (bpm BETWEEN 20 AND 300),
    ...
    is_variable_tempo  boolean    NOT NULL DEFAULT false,
    is_canonical       boolean    NOT NULL DEFAULT false,  -- какой grid использовать по умолчанию
    created_at         timestamptz NOT NULL DEFAULT now(),
    updated_at         timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT beatgrid_track_source_uq UNIQUE (track_id, source_app)
);

-- FIX v6: partial unique index — максимум один canonical beatgrid на трек.
-- Ранее enforcement был "на уровне приложения" — теперь гарантируется на уровне БД.
CREATE UNIQUE INDEX idx_beatgrid_canonical
    ON dj_beatgrid (track_id) WHERE is_canonical = true;

COMMENT ON COLUMN dj_beatgrid.is_canonical IS
    'Какой grid считается основным. Один трек может иметь grids от разных источников, '
    'но is_canonical=true может быть ровно один (partial unique index idx_beatgrid_canonical).';


CREATE TABLE dj_beatgrid_change_points (
    point_id      bigint     GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    beatgrid_id   bigint     NOT NULL REFERENCES dj_beatgrid ON DELETE CASCADE,
    position_ms   integer    NOT NULL CHECK (position_ms >= 0),
    bpm           real       NOT NULL CHECK (bpm BETWEEN 20 AND 300),
    created_at    timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX idx_beatgrid_cp_grid ON dj_beatgrid_change_points (beatgrid_id, position_ms);
...
-- FIX v5: индекс для выборки версий по сету
CREATE INDEX idx_set_versions_set ON dj_set_versions (set_id);
...
-- FIX v5: индекс для выборки constraints по версии сета
CREATE INDEX idx_set_constraints_version ON dj_set_constraints (set_version_id);


-- FIX v5: dj_set_items — composite FK для section_id + track_id
-- Аналогично transitions: section_id ОБЯЗАН принадлежать соответствующему track_id.
CREATE TABLE dj_set_items (
    ...
    -- FIX v5: composite FK — section_id ОБЯЗАН принадлежать track_id
    CONSTRAINT fk_set_item_in_section FOREIGN KEY (in_section_id, track_id)
        REFERENCES track_sections (section_id, track_id) ON DELETE SET NULL,
    CONSTRAINT fk_set_item_out_section FOREIGN KEY (out_section_id, track_id)
        REFERENCES track_sections (section_id, track_id) ON DELETE SET NULL
);

-- FIX v5: индексы для dj_set_items
CREATE INDEX idx_set_items_version ON dj_set_items (set_version_id);
CREATE INDEX idx_set_items_track ON dj_set_items (track_id);


-- ════════════════════════════════════════════════════════════════════════════
-- 17. DJ Set feedback (обратная связь)
-- ════════════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════════════
-- 19. Functions: key distance computation
-- ════════════════════════════════════════════════════════════════════════════

-- FIX v5: IMMUTABLE → STABLE — функция читает из key_edges (таблица),
-- поэтому результат может измениться при изменении данных.
-- IMMUTABLE подразумевает "зависит ТОЛЬКО от аргументов", что нарушается SELECT.
...
-- FIX v5: IMMUTABLE → STABLE (наследует от camelot_distance)
...
-- FIX v4: DROP IF EXISTS перед CREATE для идемпотентности при повторных прогонах
DO $$
DECLARE
    tbl text;
BEGIN
...
-- ════════════════════════════════════════════════════════════════════════════
-- 21. Row-level security prep (опционально, для multi-user)
-- ════════════════════════════════════════════════════════════════════════════
